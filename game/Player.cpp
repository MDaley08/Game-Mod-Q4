...
// MICHAEL MOD START
	else if (!idStr::Icmp(statname, "undying") && !checkOnly) {
		GivePowerUp(owner, POWERUP_UNDYING, SEC2MS(atof(value)));
	}
	else if (!idStr::Icmp(statname, "berserker") && !checkOnly) {
		GivePowerUp(owner, POWERUP_BERSERKER, SEC2MS(atof(value)));
	}
	else if (!idStr::Icmp(statname, "poltergeist") && !checkOnly) {
		GivePowerUp(owner, POWERUP_POLTERGEIST, SEC2MS(atof(value)));
	}
	else if (!idStr::Icmp(statname, "destroyerr") && !checkOnly) {
		GivePowerUp(owner, POWERUP_DESTROYER, SEC2MS(atof(value)));
	}
	else if (!idStr::Icmp(statname, "speedster") && !checkOnly) {
		GivePowerUp(owner, POWERUP_SPEEDSTER, SEC2MS(atof(value)));
	}
// MICHAEL MOD END
...
// MICHAEL MOD START
	if (PowerUpActive(POWERUP_BERSERKER)) {
		switch (type) {
		case PMOD_PROJECTILE_DAMAGE: {
			mod *= 3.0f;
			break;
	...
	if (PowerUpActive(POWERUP_POLTERGEIST)) {
		switch (type) {
		case PMOD_PROJECTILE_DAMAGE: {
			mod *= 3.0f;
			break;
	...
	if (PowerUpActive(POWERUP_DESTROYER)) {
		switch (type) {
		case PMOD_PROJECTILE_DAMAGE: {
			mod *= 5.0f;
			break;
	...
	if (PowerUpActive(POWERUP_SPEEDSTER)) {
		switch (type) {
		case PMOD_PROJECTILE_DAMAGE: {
			mod *= 1.2f;
			break;
...
void idPlayer::StopPowerUpEffect(int powerup) {
	...
	if (!(
		...
		(inventory.powerups & (1 << POWERUP_POLTERGEIST))
		)) {
...
// MICHAEL MOD START
		if (PowerUpActive(POWERUP_BERSERKER)) { //ammo regen for berserker
			for (int i = 0; i < MAX_WEAPONS; i++) {
				if (inventory.weapons & (1 << i) && PowerUpActive(POWERUP_BERSERKER)) {
					...
					if (gameLocal.time > nextAmmoRegenPulse[ammoIndex] && inventory.ammo[ammoIndex] < max) {
						int step = inventory.AmmoRegenStepForWeaponIndex(i * 2);
						int time = inventory.AmmoRegenTimeForWeaponIndex(i / 2);
		...
		if (gameLocal.time > nextHealthPulse) {
			if (health > 0) {
				...
				if (PowerUpActive(POWERUP_UNDYING)) { //regen for undying powerup 
					int healthTic = 40;
				...
				if (PowerUpActive(POWERUP_SPEEDSTER)) { //regen for speedster powerup
					int healthTic = 10;
					...
					else if (health > (healthBoundary * 1.2)) {
						health += healthTic / 5;
						nextHealthPulse = gameLocal.time + HEALTH_PULSE;
					}
				}
